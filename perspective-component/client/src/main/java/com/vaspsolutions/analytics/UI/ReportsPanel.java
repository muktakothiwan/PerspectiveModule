...
public class ReportsPanel extends JPanel implements TreeSelectionListener,ActionListener, MouseListener {
	
	private ModuleRPC rpc;
	private String project;
	private String dataSource;
	
	AlarmSummaryReportPanel alarmSummaryPanel;
	...
	private boolean reportForAllProjects = true;
	...
	public ReportsPanel(ModuleRPC _rpc, String _project, String _datasource) {

		/*
		 * FOLLOWING IS EXAMPLE CODE TO RETRIEVE THE DEFAULT LOOK AND FEEL CLASS 
		 * AS WELL AS DEFAULT PROPERTIES OF THE LOOK AND FEEL 
		 
		System.out.println("default look and feel :"  + UIManager.getLookAndFeel().getClass());
		
		UIDefaults _defaults = UIManager.getLookAndFeel().getDefaults();
		Set<Entry<Object,Object>> defaultSet = _defaults.entrySet();
		Iterator<Entry<Object,Object>> itr = defaultSet.iterator();
		while(itr.hasNext())
		{
			Entry<Object,Object> oneEntry =itr.next(); 
			System.out.println( oneEntry.getKey()+ " : " + oneEntry.getValue()); 
		}
		
		Enumeration<Object> _defaultObjs = _defaults.elements();
		while(_defaultObjs.hasMoreElements())
		{
			System.out.println("elem : " + _defaultObjs.nextElement());
		}
        ...
        */ 
		...
		if(project != null && (project.compareToIgnoreCase("All Projects") != 0))
		{
		...
		this.noOfPeople = rpc.getNumberOfActiveUsers(constantDuration, project, reportForAllProjects);
		//set the layout
		
		GridBagLayout gridBagLayout = new GridBagLayout();
		gridBagLayout.columnWidths = new int[]{200, 1500, 0};
		gridBagLayout.rowHeights = new int[]{1070};
		gridBagLayout.columnWeights = new double[]{Double.MIN_VALUE, 1.0, Double.MIN_VALUE};
		...
		categoryPanel.setBorder(new MatteBorder(0, 0, 0, 1, (Color) Color.LIGHT_GRAY));
		...
		headerPanel.setBorder(new MatteBorder(1, 0, 1, 0, (Color) Constants.COLOR_GRADIENT_LINE));
		...
		reportDetails.getViewport().setBackground(Constants.COLOR_WHITE_BACKGROUND);
...
//		alarmSummaryPanel = new AlarmSummaryReportPanel(alarmSummaryTable,rpc,constantDuration,reportForAllProjects,project);
//		//AlarmSummaryReportPanel(ReportsTable reportTable, ModuleRPC rpc,int duration,boolean allProjects, String projecName)
//		//rpc.getAlarmsSummaryReport(project, reportForAllProjects, constantDuration);
//		this.reportDetails.setViewportView(alarmSummaryPanel);
	...
	private void initializeMenu()
	{
		...
		DefaultMutableTreeNode topNodeOverview =
		        new DefaultMutableTreeNode("<html><b>&nbsp;Overview</b><br></html>");
		...
		DefaultMutableTreeNode topNodeSystems =
		        new DefaultMutableTreeNode("<html><br><b>&nbsp;Systems</b><br></html>");
		...
		DefaultMutableTreeNode topNodebehaviour =
		        new DefaultMutableTreeNode("<html><br><b>&nbsp;Behavior</b><br></html>");
		...
		DefaultMutableTreeNode topNodealarms = 
		        new DefaultMutableTreeNode("<html><br><b>&nbsp;Alarms</b><br></html>");
...
//		r.setOpaque(false);
//		r.setBackground(Constants.COLOR_WHITE_BACKGROUND);
//		r.setBackgroundNonSelectionColor(Constants.COLOR_WHITE_BACKGROUND);
//		r.setBackgroundSelectionColor(Constants.COLOR_BLUE_LABEL);
//		r.setBorderSelectionColor(Constants.COLOR_BLUE_LABEL);
//	//	r.setLeafIcon(treeNodeIcon);
//////		r.setClosedIcon(new ImageIcon(getClass().getResource("treeNodeClose.png")));
//////		r.setOpenIcon(new ImageIcon(getClass().getResource("treeNodeOpen.png")));
//		r.setTextNonSelectionColor(Color.BLACK);
//		r.setTextSelectionColor(Color.white);
		...
		gbl_categoryPanel.rowWeights = new double[]{0.0, 0.0, Double.MIN_VALUE};
	...
	public void refreshReport(String projectName)
	{
	...
	private void addReport()
	{
		...
		if(selectedReportMenu.compareToIgnoreCase("By Date") == 0)
		{
			...
			Dataset _overview = rpc.reoprtsGetOverviewByDate( constantDuration, this.project, this.reportForAllProjects);
			...
			if(_overview != null)
			{
				...
				else
				{
					...
					for(i=startVal; i<noOfRows; i++)
					{
						...
						if(constantDuration != Constants.TODAY && constantDuration != Constants.YESTERDAY
							&& constantDuration != Constants.LAST_365_DAYS && constantDuration != Constants.THIS_YEAR
...
//				{
			...
			//	overviewData.addRow(new Object[]{ noOfRows + " Result(s)", this.noOfPeople , t_val2  , t_val3 });
			//	summaryRow.addRow(new Object[]{ noOfRows + " Result(s)", this.noOfPeople + " People", t_val2 + " Visits", t_val3 + " Actions"});
		...
		else if(selectedReportMenu.compareToIgnoreCase("Cities") == 0)
		{
			...
			reportData = rpc.getCitiesReportData(project, reportForAllProjects, constantDuration);
...
//			{
		...
		{ 
			...
			List<GroupReportRecord> grpReportData = rpc.getGroupsReportData(project, reportForAllProjects, constantDuration);
			...
			size = grpReportData.size();
			String tempVal;
			
			DefaultTableModel groupsTableModel = new DefaultTableModel();
			groupsTableModel.setColumnIdentifiers(new Object[]{"Group", " People", "Visits", "Actions"});
			int totalNoOfPeople = 0;
			int totalNoOfVisits = 0;
			int totalNoOfActions = 0;
			int peopleVal = 0;
			...
			if(size > 0)
			{
				...
				OverviewInformation _overview = rpc.getOverview(constantDuration, project, reportForAllProjects);
				
				groupsTableModel.addRow(new Object[]{size + " Result(s)", this.noOfPeople + " People",_overview.getNoOfSessions() + " Visits",_overview.getNoOfActions() + " Actions"});
...
//				}
//			}
			addTableView(groupsTableModel, 3, "People", "Visits", "Actions");
			//Draw Graph
		...
		else if(selectedReportMenu.compareToIgnoreCase("Top Screens") == 0)
		{
			...
			reportData = rpc.getTopScreens( constantDuration, this.project, this.reportForAllProjects);
			//set the tabular view
			DefaultTableModel screensData = new DefaultTableModel(0,3);
...
//			     {
			     ...
			     //}
			     screensData.setColumnIdentifiers(new Object[] {"Page Views", "People", "Actions"});
			  
			     addTableView(screensData, 2 , "People", "Actions", "");
...
//			        
		...
		else if(selectedReportMenu.compareToIgnoreCase("Bounce Rate") == 0)
		{
			...
			Dataset bRateData = rpc.getBounceRateReportByDate(constantDuration, this.project, this.reportForAllProjects);
			DefaultTableModel overviewData = new DefaultTableModel(0,3);
			overviewData.setColumnIdentifiers(new Object[] {"By Date", "People", "Visits", "Bounce Rate"});
			//create a line chart
			DefaultCategoryDataset _lineData = new DefaultCategoryDataset();
		
			float bounceRate = rpc.getBounceRate(constantDuration, this.project, this.reportForAllProjects);
			SimpleDateFormat df = new SimpleDateFormat("EE MMM dd HH:mm:ss z yyyy");
			SimpleDateFormat sdf = new SimpleDateFormat("EE, MMM dd, yyyy");
			...
			if(bRateData != null)
			{
				...
				else
				{
					for(i=noOfRows-1; i>=0; i--)
					{
						...
						String dateVal = bRateData.getValueAt(i, 0).toString();
						if(constantDuration != Constants.TODAY && constantDuration != Constants.YESTERDAY && constantDuration != Constants.LAST_365_DAYS && constantDuration != Constants.THIS_YEAR && constantDuration != Constants.LAST_YEAR){
							try {
								xDate = df.parse(dateVal);
...
//				_lineChart = ChartFactory.createLineChart("", "", "", _lineData, PlotOrientation.VERTICAL, false, true, true );
		...
		else if(selectedReportMenu.compareToIgnoreCase("Device Types") == 0)
		{
			...
			reportData = rpc.getDeviceTypeReport( constantDuration, this.project, this.reportForAllProjects);
			...
			if(reportData !=null){
				...
				for (int i = 0 ; i< noOfRecords ; i++ ){
					...
					if(reportData.getValueAt(i, 4) != null)
					{
						...
						devicesData.addRow(new Object[]{category,noOfPeople,noOfvisits, noOfActions });
...
//				}
		...
		else if(selectedReportMenu.compareToIgnoreCase("Platforms") == 0)
		{
			...
			reportData = rpc.getPlatformReport( constantDuration, this.project, this.reportForAllProjects);
			DefaultTableModel platformsData = new DefaultTableModel(0,3);
			platformsData.setColumnIdentifiers(new Object[] {"Platforms", "People", "Visits", "Actions"});
			...
			if(reportData !=null){
				...
				for (int i = 0 ; i< noOfRecords ; i++ ){
					...
					platformsData.addRow(new Object[]{category,noOfPeople,noOfvisits, noOfActions });
			...
			//	if(noOfRecords > 0){
					platformsData.addRow(new Object[]{noOfRecords + " Result(s)",this.noOfPeople + " People",totalNoOfVisits + " Visits", totalNoOfActions + " Actions"});
			//	}
				//to create dummy records in case dataset is less.
				if(noOfRecords < 10)
				{
		...
		else if(selectedReportMenu.compareToIgnoreCase("Browsers") == 0)
		{
			...
			int noOfUsers = rpc.getDistinctUsersFromBrowsers(constantDuration, this.project, this.reportForAllProjects);
			
			reportData = rpc.getBrowserReport( constantDuration, this.project, this.reportForAllProjects);
			DefaultTableModel browsersData = new DefaultTableModel(0,3);
			browsersData.setColumnIdentifiers(new Object[] {"Browsers", "People", "Visits", "Actions"});
			...
			if(reportData !=null )
			{
				...
				for (int i = 0 ; i< noOfRecords ; i++ ){
					...
					if(reportData.getValueAt(i, 3) != null)
					{
						noOfPeople = (int)Float.parseFloat(reportData.getValueAt(i, 3).toString());
					}
					if(reportData.getValueAt(i, 2) != null)
					{
						noOfvisits = (int)Float.parseFloat(reportData.getValueAt(i, 2).toString());
					...
					if(reportData.getValueAt(i, 1) != null)
					{
						noOfActions = (int)Float.parseFloat(reportData.getValueAt(i, 1).toString());
					}
					if(reportData.getValueAt(i, 4) != null)
					{
						noOfActions = noOfActions + (int)Float.parseFloat(reportData.getValueAt(i, 4).toString());
					...
					browsersData.addRow(new Object[]{category,noOfPeople,noOfvisits, noOfActions });
					totalNoOfPeople = totalNoOfPeople + noOfPeople;
					totalNoOfVisits = totalNoOfVisits + noOfvisits;
					totalNoOfActions = totalNoOfActions  + noOfActions;
				}
		...
		else if(selectedReportMenu.compareToIgnoreCase("Screen Resolutions") == 0)
		{
			...
			reportData = rpc.getScreenResolutionData(project, reportForAllProjects, constantDuration);
			...
			String tempVal;
			 	...
			 	DefaultTableModel screenDepthTableModel = new DefaultTableModel();
			 	screenDepthTableModel.setColumnIdentifiers(new Object[]{"Screen Resolution", " People", "Visits", "Actions"});
		...
		else if(selectedReportMenu.compareToIgnoreCase("Actions Per Visit") == 0)
		{
			...
			reportData = rpc.getActionsPerVisitReportInformation(project, reportForAllProjects, constantDuration);
			...
			for(int i = 0 ; i < size ; i ++){
				...
				if(tempVal.compareToIgnoreCase("NULL") == 0){
					...
					if(tempVal.compareToIgnoreCase("1-5") == 0){
				...
				if(tempVal.compareToIgnoreCase("6-10") == 0){
				...
				if(tempVal.compareToIgnoreCase("11-20") == 0){
					...
					if(tempVal.compareToIgnoreCase("21-30") == 0){
					...
					if(tempVal.compareToIgnoreCase("31-50") == 0){
					...
					if(tempVal.compareToIgnoreCase("51 or more") == 0){
			...
			dataset.addValue(actionPerVisit_6to10_People, series1, "6 to 10");
			dataset.addValue(actionPerVisit_6to10_Visits, series2, "6 to 10");
			dataset.addValue(actionPerVisit_6to10_Actions, series3, "6 to 10");

			dataset.addValue(actionPerVisit_10to20_People, series1, "11 to 20");
			dataset.addValue(actionPerVisit_10to20_Visits, series2, "11 to 20");
			dataset.addValue(actionPerVisit_10to20_Actions, series3, "11 to 20");
			
			dataset.addValue(actionPerVisit_21to30_People, series1, "21 to 30");
			dataset.addValue(actionPerVisit_21to30_Visits, series2, "21 to 30");
			dataset.addValue(actionPerVisit_21to30_Actions, series3, "21 to 30");
			
			dataset.addValue(actionPerVisit_31to50_People, series1, "31 to 50");
			dataset.addValue(actionPerVisit_31to50_Visits, series2, "31 to 50");
			dataset.addValue(actionPerVisit_31to50_Actions, series3, "31 to 50");
			
			dataset.addValue(actionPerVisit_51plus_People, series1, "51 or More");
			dataset.addValue(actionPerVisit_51plus_Visits, series2, "51 or More");
			dataset.addValue(actionPerVisit_51plus_Actions, series3, "51 or More");
			...
			//Data for table 
			 	DefaultTableModel screenDepthTableModel = new DefaultTableModel();
			 	screenDepthTableModel.setColumnIdentifiers(new Object[]{"Actions Per Visit", " People", "Visits", "Actions"});
			     
//				 if(size > 0)
//				 {
					 screenDepthTableModel.addRow(new Object[]{"1 to 5", actionPerVisit_1_5People,actionPerVisit_1_5Visits,actionPerVisit_1_5Actions});
					 screenDepthTableModel.addRow(new Object[]{"6 to 10", actionPerVisit_6to10_People,actionPerVisit_6to10_Visits,actionPerVisit_6to10_Actions});
					 screenDepthTableModel.addRow(new Object[]{"11 to 20", actionPerVisit_10to20_People,actionPerVisit_10to20_Visits,actionPerVisit_10to20_Actions});
					 screenDepthTableModel.addRow(new Object[]{"21 to 30", actionPerVisit_21to30_People,actionPerVisit_21to30_Visits,actionPerVisit_21to30_Actions});
					 screenDepthTableModel.addRow(new Object[]{"31 to 50", actionPerVisit_31to50_People,actionPerVisit_31to50_Visits,actionPerVisit_31to50_Actions});
					 screenDepthTableModel.addRow(new Object[]{"51 or More", actionPerVisit_51plus_People,actionPerVisit_51plus_Visits,actionPerVisit_51plus_Actions});
					
						totalPeople = actionPerVisit_1_5People + actionPerVisit_6to10_People + 
								actionPerVisit_10to20_People + actionPerVisit_21to30_People + actionPerVisit_31to50_People
		...
		else if(selectedReportMenu.compareToIgnoreCase("Visit Duration") == 0)
		{
			...
			reportData = rpc.getVisitDurationReportInformation(project, reportForAllProjects, constantDuration);
			...
			if(tempVal.compareToIgnoreCase("120") == 0){
			...
			if(tempVal.compareToIgnoreCase("31-40") == 0){
			...
			if(tempVal.compareToIgnoreCase("41-50") == 0){
			...
			if(tempVal.compareToIgnoreCase("51-60") == 0){
			...
			if(tempVal.compareToIgnoreCase("61-120") == 0){
	      ...
	      //Data for table 
			 	...
			 	visitDutrationTableModel.setColumnIdentifiers(new Object[]{"Visit Duration", " People", "Visits", "Actions"});
			     
					 visitDutrationTableModel.addRow(new Object[]{"0-5 minutes", people_0_5,visits_0_5,actions_0_5});
					 visitDutrationTableModel.addRow(new Object[]{"6-10 minutes", people_6_10,visits_6_10,actions_6_10});
					 visitDutrationTableModel.addRow(new Object[]{"11-20 minutes", people_11_20,visits_11_20,actions_11_20});
					 visitDutrationTableModel.addRow(new Object[]{"21-30 minutes", people_21_30,visits_21_30,actions_21_30});
					 visitDutrationTableModel.addRow(new Object[]{"31-40 minutes", people_31_40,visits_31_40,actions_31_40});
					 visitDutrationTableModel.addRow(new Object[]{"41-50 minutes", people_41_50,visits_41_50,actions_41_50});
					 visitDutrationTableModel.addRow(new Object[]{"51 minutes-1 hour", people_51_60,visits_51_60,actions_51_60});
					 visitDutrationTableModel.addRow(new Object[]{"1 hour-2 hours", people_61_120,visits_61_120,actions_61_120});
					 visitDutrationTableModel.addRow(new Object[]{"2+ hours", people_120,visits_120,actions_120});
					 
					 //compute the summary row 
				int totalPeople = people_0_5 + people_6_10 + people_11_20 + people_21_30 + people_31_40
						+ people_41_50 + people_51_60 + people_61_120 + people_120;
				...
				visitDutrationTableModel.addRow(new Object[]{"", this.noOfPeople + " People",totalVisits + " Visits",totalActions + " Actions"});
					 ...
					 addTableView(visitDutrationTableModel, 3, "People", "Visits", "Actions");
			
		}
		else if(selectedReportMenu.compareToIgnoreCase("Engagement") == 0)
		{
			this.lblreportTitle.setText("Engagement");
			reportData = rpc.getEngagementReportInformationScreenDepth(project, reportForAllProjects, constantDuration);
			...
			//Data for table 
			 	DefaultTableModel screenDepthTableModel = new DefaultTableModel();
			 	screenDepthTableModel.setColumnIdentifiers(new Object[]{"Depth of Screen Viewed", " People", "Visits", "Actions"});
			     
//				 if(size > 0)
//				 {
					 screenDepthTableModel.addRow(new Object[]{"1 screen", Screen_1_People,Screen_1_Visits,Screen_1_Actions});
					 screenDepthTableModel.addRow(new Object[]{"2 screens", Screen_2_People,Screen_2_Visits,Screen_2_Actions});
					 screenDepthTableModel.addRow(new Object[]{"3 screens", Screen_3_People,Screen_3_Visits,Screen_3_Actions});
					 screenDepthTableModel.addRow(new Object[]{"4 screens", Screen_4_People,Screen_4_Visits,Screen_4_Actions});
					 screenDepthTableModel.addRow(new Object[]{"5 screens", Screen_5_People,Screen_5_Visits,Screen_5_Actions});
					 screenDepthTableModel.addRow(new Object[]{"6-10 screens", Screen_6to10_People,Screen_6to10_Visits,Screen_6to10_Actions});
					 screenDepthTableModel.addRow(new Object[]{"11-15 screens", Screen_11to15_People,Screen_11to15_Visits,Screen_11to15_Actions});
					 screenDepthTableModel.addRow(new Object[]{"16-20 screens", Screen_16to20_People,Screen_16to20_Visits,Screen_16to20_Actions});
					 screenDepthTableModel.addRow(new Object[]{"20+ screens", Screen_20Plus_People,Screen_20Plus_Visits,Screen_20Plus_Actions});
			//	 } 
				 
				 int totalPeople = Screen_1_People + Screen_2_People + Screen_3_People + Screen_4_People + Screen_5_People
							+ Screen_6to10_People + Screen_11to15_People + Screen_16to20_People + Screen_20Plus_People;
		...
		else if(selectedReportMenu.compareToIgnoreCase("Frequency") == 0)
		{
			...
			reportData = rpc.getFrequencytReportInformation(project, reportForAllProjects, constantDuration);
	        ...
	        //Data for table
		 	...
		 	DefaultTableModel screenDepthTableModel = new DefaultTableModel();
		 	screenDepthTableModel.setColumnIdentifiers(new Object[]{"Count of sessions", " People", "Visits", "Actions"});
		     
//			 if(size > 0)
//			 {
				 screenDepthTableModel.addRow(new Object[]{"1 session", Session_1_People,Session_1_Visits,Session_1_Actions});
				 screenDepthTableModel.addRow(new Object[]{"2 sessions", Session_2_People,Session_2_Visits,Session_2_Actions});
				 screenDepthTableModel.addRow(new Object[]{"3 sessions", Session_3_People,Session_3_Visits,Session_3_Actions});
				 screenDepthTableModel.addRow(new Object[]{"4 sessions", Session_4_People,Session_4_Visits,Session_4_Actions});
				 screenDepthTableModel.addRow(new Object[]{"5 sessions", Session_5_People,Session_5_Visits,Session_5_Actions});
				 screenDepthTableModel.addRow(new Object[]{"6-10 sessions", Session_6to10_People,Session_6to10_Visits,Session_6to10_Actions});
				 screenDepthTableModel.addRow(new Object[]{"11-15 sessions", Session_11to15_People,Session_11to15_Visits,Session_11to15_Actions});
				 screenDepthTableModel.addRow(new Object[]{"16-20 sessions", Session_16to20_People,Session_16to20_Visits,Session_16to20_Actions});
				 screenDepthTableModel.addRow(new Object[]{"20+ sessions", Session_20Plus_People,Session_20Plus_Visits,Session_20Plus_Actions});
		//	 } 
			 int totalPeople = Session_1_People + Session_2_People + Session_3_People + Session_4_People + Session_5_People
						+ Session_6to10_People + Session_11to15_People + Session_16to20_People + Session_20Plus_People;
		...
		else if(selectedReportMenu.compareToIgnoreCase("Recency") == 0)
		{
			...
			reportData = rpc.getRecencytReportInformation(project, reportForAllProjects, constantDuration);
			...
			for(int i = 0 ; i < size ; i ++){
				...
				if(tempVal <= 1 ){
					...
					Recency_1_Actions = Recency_1_Actions + (int)Float.parseFloat(reportData.getValueAt(i , "Actions").toString())
							+ (int)Float.parseFloat(reportData.getValueAt(i , "noOfScreens").toString());
					String testing = reportData.getValueAt(i , "Visits").toString();
				...
				if(tempVal >20 ){
					...
					Recency_20Plus_Actions = Recency_20Plus_Actions + (int)Float.parseFloat(reportData.getValueAt(i , "Actions").toString())
							+ (int)Float.parseFloat(reportData.getValueAt(i , "noOfScreens").toString());
					String testing = reportData.getValueAt(i , "Visits").toString();
					Recency_20Plus_Visits = Recency_20Plus_Visits + (int)Float.parseFloat(testing);
				}
	        ...
	        //Data for table
		 	...
		 	DefaultTableModel screenDepthTableModel = new DefaultTableModel();
		 	screenDepthTableModel.setColumnIdentifiers(new Object[]{"Days Since Last Session", " People", "Visits", "Actions"});
		     
			 if(size > 0)
			 {
				 screenDepthTableModel.addRow(new Object[]{"1 day", Recency_1_People,Recency_1_Visits,Recency_1_Actions});
				 screenDepthTableModel.addRow(new Object[]{"2 days", Recency_2_People,Recency_2_Visits,Recency_2_Actions});
				 screenDepthTableModel.addRow(new Object[]{"3 days", Recency_3_People,Recency_3_Visits,Recency_3_Actions});
				 screenDepthTableModel.addRow(new Object[]{"4 days", Recency_4_People,Recency_4_Visits,Recency_4_Actions});
				 screenDepthTableModel.addRow(new Object[]{"5 days", Recency_5_People,Recency_5_Visits,Recency_5_Actions});
				 screenDepthTableModel.addRow(new Object[]{"6-10 days", Recency_6to10_People,Recency_6to10_Visits,Recency_6to10_Actions});
				 screenDepthTableModel.addRow(new Object[]{"11-15 days", Recency_11to15_People,Recency_11to15_Visits,Recency_11to15_Actions});
				 screenDepthTableModel.addRow(new Object[]{"16-20 days", Recency_16to20_People,Recency_16to20_Visits,Recency_16to20_Actions});
				 screenDepthTableModel.addRow(new Object[]{"20+ days", Recency_20Plus_People,Recency_20Plus_Visits,Recency_20Plus_Actions});
			 } 
			 int totalPeople = Recency_1_People + Recency_2_People + Recency_3_People + Recency_4_People + Recency_5_People
				...
				screenDepthTableModel.addRow(new Object[]{"", totalPeople + " People",totalVisits + " Visits",totalActions + " Actions"});
		...
		else if(selectedReportMenu.compareToIgnoreCase("Active Users") == 0)
		{
			...
			DefaultCategoryDataset dataset = new DefaultCategoryDataset();
			ActiveUsersInfo aInfo = rpc.getActiveUsersCounts(project, reportForAllProjects, constantDuration);
			...
			reportData = rpc.getActiveUserDataReportGraph(this.dataSource, constantDuration, this.project, this.reportForAllProjects);//( Constants.LAST_THIRTY_DAYS, this.project, this.reportForAllProjects);
			DefaultTableModel platformsData = new DefaultTableModel(0,3);
			platformsData.setColumnIdentifiers(new Object[] {"By Date", "1 Day active Users", "7 Day active Users", "14 Day active Users"});
			SimpleDateFormat sdf = new SimpleDateFormat("EE, MMM dd, yyyy");
			SimpleDateFormat df = new SimpleDateFormat("EE MMM dd HH:mm:ss z yyyy");
			...
			if(reportData !=null){
				...
				else
				{
					...
					if(constantDuration != Constants.LAST_365_DAYS && constantDuration != Constants.TODAY && constantDuration != Constants.YESTERDAY && constantDuration != Constants.LAST_YEAR && constantDuration != Constants.THIS_YEAR){
					if(reportData.getValueAt(i, 0) != null)
					{
				...
				if(constantDuration == Constants.LAST_365_DAYS)
				{
					for(int i= startVal; i<noOfRecords; i++)
					{
						...
						if(reportData.getValueAt(i, 3) != null)
						{
							...
							dataset.addValue((int)oneDayActive, series1, categoryGraph);
							dataset.addValue((int)sevenDayActive, series2, categoryGraph);
							dataset.addValue((int)fourteenDayActive, series3, categoryGraph);
				...
				else
				{
					...
					//_lineChart = ChartFactory.createLineChart("", "", "", dataset, PlotOrientation.VERTICAL, false, true, true );
				}
					
			}
			addTableView(platformsData, 3, "1-Day Active Users", "7-Day Active Users", "14-Day Active Users");
		...
		else if(selectedReportMenu.compareToIgnoreCase("Alarm Summary") == 0)
		{
			...
			Dataset _alarmsClearTime = rpc.getAlarmsClearTime(constantDuration, this.project, this.reportForAllProjects);
			//set value in the UI
			//set the value in labels on the UI
					...
					//get ack times
					Dataset _alarmsAckTime = rpc.getAlarmsAckTime(constantDuration, this.project, this.reportForAllProjects);
					//set value in the UI
					//set the value in labels on the UI
							int alarmsAckDSSize;
			...
			reportData = rpc.getAlarmsSummaryReport(project, reportForAllProjects, constantDuration);
			...
			platformsData.setColumnIdentifiers(new Object[] {"Alarms", "Alarm Priority", "Quantity", "Average Time to Acknowledge","Average Time To Clear"});
...
//					avgTimeClr = avgTimeClr.substring(11, 16);
				...
				platformsData.addRow(new Object[]{alarmName, priority,sumAlarmCount,avgTimeAck,avgTimeClr});
			}
          ...
          upperTable.setVisible(true);
		   ...
		   upperTable.setEnabled(false);
			...
			alarmSummaryTable.setEnabled(false);
		     ...
		     alarmSummaryTable.getTableHeader().setBorder(BorderFactory.createMatteBorder(1, 0, 1, 0, Color.LIGHT_GRAY));
				...
				alarmSummaryTable.getColumnModel().getColumn(1).setCellRenderer( centerRenderer );
				...
				alarmSummaryTable.getColumnModel().getColumn(0).setPreferredWidth(600);
				alarmSummaryTable.getColumnModel().getColumn(1).setPreferredWidth(100);
				alarmSummaryTable.getColumnModel().getColumn(2).setPreferredWidth(100);
				alarmSummaryTable.getColumnModel().getColumn(3).setPreferredWidth(100);
				alarmSummaryTable.getColumnModel().getColumn(4).setPreferredWidth(100);
...
//					TableRowSorter<TableModel> sorter = new TableRowSorter<TableModel>(alarmSummaryTable.getModel());
//					alarmSummaryTable.setRowSorter(sorter);
//					List<RowSorter.SortKey> sortKeys = new ArrayList<SortKey>();
//					 
//					sortKeys.add(new RowSorter.SortKey(columnSelected, SortOrder.ASCENDING));
//					 
...
//        domainAxis1.setCategoryLabelPositions(new CategoryLabelPositions(new CategoryLabelPosition(RectangleAnchor.BOTTOM, TextBlockAnchor.CENTER_LEFT
    ...
    //    domainAxis1.setTickLabelFont(new Font(Font.DIALOG, Font.PLAIN, 11));
     //   domainAxis.setTickLabelPaint(Constants.COLOR_PLOT_LABEL);
        ...
        CategoryPlot plot = new CategoryPlot(
        		_data, domainAxis1, rangeAxis1, renderer
        );
...
//				    }
	...
	{
		TableRowSorter<TableModel> sorter = new TableRowSorter<TableModel>(currentTable.getModel());
...
//		if(columnSelected == 1)
//		{
//			SortOrder order = SortOrder.values()["Low","Medium","High","Critical"];
...
//		{
		 ...
		 if(isRefresh == true)
		 {
			...
			{
					sortKeys.add(new RowSorter.SortKey(columnSelected, SortOrder.DESCENDING));
			}
		 }
		 else
		 {
