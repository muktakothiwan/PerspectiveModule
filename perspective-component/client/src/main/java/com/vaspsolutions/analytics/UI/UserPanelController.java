...
public class UserPanelController extends JPanel implements ActionListener, ItemListener {
	...
	public String gatewaySelectedUser;
	...
	//new changes
	public boolean allGateways = true;
	...
	private JLabel lblLblgatewayvalue;
	Dataset allUsers = null;
	HashMap<String,Integer> allUsersMap;
	
	private boolean showActions = false;
	@SuppressWarnings("null")
	public UserPanelController(ModuleRPC rpc, String ds, String projectName, String gatewayName) {
		super();
		System.out.println("user panel inputs : project name " + projectName +  ", gatewayname : " + gatewayName);
	...
	//	searchBoxPanel.add(refreshListBtn, gbc_refreshListBtn);
		...
		lblActions.setHorizontalAlignment(SwingConstants.RIGHT);
		lblActions.setBackground(Color.WHITE);
		lblActions.setForeground(new Color(0,191,245));
		GridBagConstraints gbc_lblActions = new GridBagConstraints();
		gbc_lblActions.fill = GridBagConstraints.BOTH;
		gbc_lblActions.insets = new Insets(0, 0, 5, 5);
		gbc_lblActions.gridx = 2;
		gbc_lblActions.gridy = 0;
		searchBoxPanel.add(lblActions, gbc_lblActions);
		
		checkbox = new JCheckBox("");
		checkbox.setBackground(Color.WHITE);
		checkbox.setForeground(new Color(0,191,245));
		checkbox.setBorder(new LineBorder(new Color(0,191,245)));
		GridBagConstraints gbc_checkbox = new GridBagConstraints();
		gbc_checkbox.insets = new Insets(5, 5, 5, 0);
		gbc_checkbox.gridx = 3;
		gbc_checkbox.gridy = 0;
		searchBoxPanel.add(checkbox, gbc_checkbox);
		...
		lblGateway.setBackground(Color.WHITE);
		GridBagConstraints gbc_lblGateway = new GridBagConstraints();
		gbc_lblGateway.anchor = GridBagConstraints.EAST;
		lblGateway.setForeground(new Color(109, 110, 112));
		gbc_lblGateway.insets = new Insets(0, 0, 5, 5);
		gbc_lblGateway.gridx = 1;
		gbc_lblGateway.gridy = 13;
		profileInfoPanel.add(lblGateway, gbc_lblGateway);
		
		lblLblgatewayvalue = new JLabel("");
		lblLblgatewayvalue.setBackground(Color.WHITE);
		GridBagConstraints gbc_lblLblgatewayvalue = new GridBagConstraints();
		gbc_lblLblgatewayvalue.anchor = GridBagConstraints.WEST;
		gbc_lblLblgatewayvalue.insets = new Insets(0, 0, 5, 5);
		gbc_lblLblgatewayvalue.gridx = 3;
		gbc_lblLblgatewayvalue.gridy = 13;
		profileInfoPanel.add(lblLblgatewayvalue, gbc_lblLblgatewayvalue);
		...
		gbc_lblGroupValue.fill = GridBagConstraints.HORIZONTAL;
		gbc_lblGroupValue.insets = new Insets(2, 0, 5, 10);
		gbc_lblGroupValue.gridx = 3;
		gbc_lblGroupValue.gridy = 14;
		profileInfoPanel.add(lblGroupValue, gbc_lblGroupValue);
		
		label_1 = new JLabel("Group");
		label_1.setForeground(new Color(109, 110, 112));
		GridBagConstraints gbc_label_1 = new GridBagConstraints();
		gbc_label_1.anchor = GridBagConstraints.NORTHEAST;
		gbc_label_1.insets = new Insets(2, 0, 5, 5);
		gbc_label_1.gridx = 1;
		gbc_label_1.gridy = 14;
		profileInfoPanel.add(label_1, gbc_label_1);
		...
		gbc_lblSecurityValue.fill = GridBagConstraints.HORIZONTAL;
		gbc_lblSecurityValue.insets = new Insets(2, 0, 5, 10);
		gbc_lblSecurityValue.gridx = 3;
		gbc_lblSecurityValue.gridy = 15;
		profileInfoPanel.add(lblSecurityValue, gbc_lblSecurityValue);
		
		label = new JLabel("Security");
		label.setForeground(new Color(109, 110, 112));
		GridBagConstraints gbc_label = new GridBagConstraints();
		gbc_label.anchor = GridBagConstraints.NORTHEAST;
		gbc_label.insets = new Insets(2, 0, 5, 5);
		gbc_label.gridx = 1;
		gbc_label.gridy = 15;
		profileInfoPanel.add(label, gbc_label);
	...
	//	this.userProfiles = rpc.getUserProfiles();
		...
		this.allUsers = rpc.getAllLoggedInUsersOnController(allProjects, this.currentProject, this.allGateways, this.currentGateway );
		...
		DateFormat lastLogindf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss",Locale.ENGLISH);
		
		if(allUsers != null)
		{
			allUsersMap = new HashMap<String, Integer>();
			noOfUsers = allUsers.getRowCount();
			for(i=0; i<noOfUsers; i++)
			{
				loggedInStr = "";
				if(allUsers.getValueAt(i, "username") != null)
				{
					userName = allUsers.getValueAt(i, "username").toString().trim();
				}
				if(allUsers.getValueAt(i, "gateway_userprofile") != null)
				{
					profileName = allUsers.getValueAt(i, "gateway_userprofile").toString().trim();
				}
				if(allUsers.getValueAt(i, "gateway_id") != null)
				{
					gatewayID = allUsers.getValueAt(i, "gateway_id").toString().trim();
				}
				
				String hashKey = userName + ":" + profileName + ":" + gatewayID;
				allUsersMap.put(hashKey, i);
				//check if user is online 
				if(allUsers.getValueAt(i, "online_status") != null)
				{
					loggedInStr = allUsers.getValueAt(i, "online_status").toString();
				}
				
				if(loggedInStr.compareToIgnoreCase("Offline") == 0)
				{
					String lastSeen = allUsers.getValueAt(i, "last_seen").toString();
					if(lastSeen != "" )
					{
				...
				String userLocation = "";
				if(allUsers.getValueAt(i, "current_location") != null)
				{
					userLocation = allUsers.getValueAt(i, "current_location").toString();
				...
				userListPanel.lblGatewayName.setText(gatewayID);
				userListPanel.lblGatewayName.setVisible(true);
		...
		//search in userListData for selection
				if(this.selectedUser != null)
				{
					...
					for(int indx=0; indx<totalUsers; indx++)
					{
						...
						_gatewayName = this.userListData.get(indx).lblGatewayName.getText();
						if(_userName.compareToIgnoreCase(this.selectedUser) == 0 && _profileName.compareToIgnoreCase(this.profileSelectedUser) == 0
								&& _gatewayName.compareToIgnoreCase(this.gatewaySelectedUser) == 0)
						{
							selectedIndex = indx;
						}
...
//			populateUsersList();
	...
	public void userPanelPopulate(){
		...
		if(btn != -1)
		{
			...
			this.gatewaySelectedUser = testing.lblGatewayName.getText();
		
			int userIndex = this.getRecordNumberInUserList(this.selectedUser, this.profileSelectedUser, this.gatewaySelectedUser);
			userImageLbl.setVisible(true);
			//check if user from selected profile is online 
			if((this.allUsers.getValueAt(userIndex, "online_status").toString()).compareToIgnoreCase("Online") == 0){
				lblNewLabel.setBackground(new Color(34, 139, 34));
				lblNewLabel.setBorder(new CompoundBorder(new LineBorder(new Color(34, 139, 34), 1), new EmptyBorder(3, 5, 3, 5)));
			...
			this.lblEmailValue.setText(this.allUsers.getValueAt(userIndex, "email").toString());
					
			this.lblLblgatewayvalue.setText(this.gatewaySelectedUser);		
			this.lblGroupValue.setText(this.allUsers.getValueAt(userIndex, "roles").toString());
			this.lblSecurityValue.setText(this.allUsers.getValueAt(userIndex, "roles").toString());
						
			this.lblFirstSeenValue.setText(this.allUsers.getValueAt(userIndex, "first_seen").toString());
						this.lblLastSeenValue.setText(this.allUsers.getValueAt(userIndex, "last_seen").toString());
						this.lblSessionDurationValue.setText(this.allUsers.getValueAt(userIndex, "last7days_duration").toString() + " hours");
						
						this.lblTotalVisitsValue.setText(this.allUsers.getValueAt(userIndex, "last7days_visits").toString() + " visits");
						
						
						
						String locationTest = this.allUsers.getValueAt(userIndex, "current_location").toString();
						...
						this.lblScreenViewsValue.setText("" +this.allUsers.getValueAt(userIndex, "all_screen_views").toString());
						
						
						this.lblAllActionsValue.setText("" + this.allUsers.getValueAt(userIndex, "all_actions").toString());
						this.lblTotalActionsValue.setText("" + this.allUsers.getValueAt(userIndex, "last7days_actions").toString() + " actions");
						this.lblCurrentScreenVal.setText(this.allUsers.getValueAt(userIndex, "current_screen").toString());
						
							
					
					
						screenViewsDS = rpc.getScreenViewsPerUserPerVisitOnController(Constants.LAST_365_DAYS,currentProject, allProjects,this.selectedUser, this.profileSelectedUser, this.gatewaySelectedUser, this.showActions);
						...
						while(ii < rows){
							...
							if(this.showActions)
							{
								sessionValue.sessionOSLbl.setText(screenViewsDS.getValueAt(ii, "actions").toString().trim() + " Actions");
							}
							else
							{
								sessionValue.sessionOSLbl.setText("");
							}
		...
		//					sessionValue.screenNameLbl.setText(screenName);
							...
							for (j = ii ; j < rows ; j++){
								...
								if (sessionStartDate.equalsIgnoreCase(sessionPanelStartDate) && sessionEndDate.equalsIgnoreCase(sessionPanelEndDate)){
									...
									if(screenViewsDS.getValueAt(j, "Gateway") != null)
									{
										viewdUserName = screenViewsDS.getValueAt(j, "Gateway").toString();
									}
									else
									{
										viewdUserName = "";
									...
									if(screenViewsDS.getValueAt(j, "Project") != null)
									{
										screenViewPanelData.lblViewed.setText(screenViewsDS.getValueAt(j, "Project").toString());
									}
									else
									{
										screenViewPanelData.lblViewed.setText("");
									}
									...
									if(this.showActions)
									{
										screenViewPanelData.actionLbl.setVisible(true);
										screenViewPanelData.valueLbl.setVisible(true);
										if(screenViewsDS.getValueAt(j, "action") != null)
										{
											screenViewPanelData.actionLbl.setText(screenViewsDS.getValueAt(j, "action").toString());
										}
										else
										{
											screenViewPanelData.actionLbl.setText("");
										}
										
										if(screenViewsDS.getValueAt(j, "action_value") != null)
										{
											screenViewPanelData.valueLbl.setText(screenViewsDS.getValueAt(j, "action_value").toString());
										}
										else
										{
											screenViewPanelData.valueLbl.setText("");
										}
									}
									else
									{
						...
						{
							System.out.println("screenViewsDS null");
						}
...
//		int yesterdayRow = dataYesterDay.getRowCount();
	...
	public List<IA_UserListPanel> populateUserList(String selectedItem){
		...
		else if (selected.compareToIgnoreCase("Online Users") == 0)
		{
			...
			{	
				...
				if(alluserListData.get(i).timeSinceLastSeenLbl.getText().compareToIgnoreCase("Online") == 0)
				{
	...
	int getRecordNumberInUserList(String userName, String profileName, String gatewayID)
	{
		int returnIndex = 0;
		
		String hashKey = userName.trim() + ":" + profileName.trim() + ":" + gatewayID.trim();
		if(allUsersMap.containsKey(hashKey))
		{
			returnIndex = Integer.parseInt(allUsersMap.get(hashKey).toString());
		}
		return returnIndex;
	}

	/*
	 * for listening Actions checkbox events 
	 * (non-Javadoc)
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if(e.getSource() == this.checkbox)
		{
			if (this.checkbox.isSelected())
			{
				...
				System.out.println("show actions true");
			}
			else
			{
				this.showActions = false;
				System.out.println("show actions false");
			}
			setSelectedUser();
			userPanelPopulate();
		}
