...
public class CategoryAxisSkipLabels extends CategoryAxis  { 
	...
	public CategoryAxisSkipLabels(int customTickInterval) {
		super();
		this.customTickInterval = customTickInterval;
	}
	boolean displaySkippedTickMarks = false;
	public int customTickInterval = 3;
  /*
  ...
  public List refreshTicks(Graphics2D g2, AxisState state, Rectangle2D dataArea, RectangleEdge edge) {
	...
	if (categories != null) {
		...
		while (iterator.hasNext()) {
			...
			if(categoryIndex %customTickInterval == 0)
			{
  ...
  */
  ...
  private AxisState drawLabels(Graphics2D g2, RectangleEdge edge, AxisState state, PlotRenderingInfo plotState, Rectangle2D dataArea, LabelDefs labelDefs)throws Exception {
	  ...
	  while (labelDefsIter.hasNext()) {
		  ...
		  CategoryLabelPosition position = getCategoryLabelPositions().getLabelPosition(edge);
		  System.out.println("tick no :" + tickNo);
		  if(tickNo %2 == 0)
		  {
  ...
  private Rectangle2D calculateNewDrawingRegion(int categoryIndex, List ticks, Rectangle2D dataArea, 
      ...
      if (edge == RectangleEdge.TOP) { 
        x0 = getCategoryStart(categoryIndex, ticks.size(), dataArea, edge) - 5.0 ; 
        x1 = getCategoryEnd(categoryIndex, ticks.size(), dataArea, edge) - 5.0; 
        ...
        y0 = getCategoryStart(categoryIndex, ticks.size(), dataArea, edge) - 5.0; 
        y1 = getCategoryEnd(categoryIndex, ticks.size(), dataArea, edge) - 5.0; 
...
//      x1 = x1 - 10;
  ...
  public static TextBlock createTextBlock(final String text, final Font font,
		...
		result.setLineAlignment(HorizontalAlignment.LEFT);
